...
func TestV2_9_0DeleteConversationPermissionMigration(t *testing.T) {
	...
	// schema.sql already seeds Admin with the permission, so the seeded role also
	// exercises the "already granted" path.
	if _, err := db.Exec(`INSERT INTO roles (name, description, permissions) VALUES ($1, '', $2)`,
		"Supervisor", pq.StringArray{"conversations:read", "conversations:write"}); err != nil {
		t.Fatalf("inserting role: %v", err)
	}
