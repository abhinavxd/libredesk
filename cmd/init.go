...
func initAuth(o *oidc.Manager, rd *redis.Client, i18n *i18n.I18n, dialControl ssrf.Control, users *user.Manager, settings *setting.Manager) *auth_.Auth {
	lo := initLogger("auth")
	...
	auth, err := auth_.New(auth_.Config{Providers: providers, SecureCookies: secure, SessionLifetime: sessionLifetime, RootURL: settings.GetAppRootURL}, i18n, rd, lo, dialControl, users)
	if err != nil {
		log.Fatalf("error initializing auth: %v", err)
	}
