...
func handleAIGenerateReply(r *fastglue.Request) error {
	...
	return handleAIGenerateReplyWith(r, req)
}
...
// Stackblaze-compatible aliases for the pre-v2.8 AI admin/draft endpoints.
...
func handleAIDraftReply(r *fastglue.Request) error {
	...
	return handleAIGenerateReplyWith(r, generateReplyReq{
		ConversationUUID: strings.TrimSpace(req.ConversationUUID),
		Instruction:      strings.TrimSpace(req.Instructions),
	})
}

func handleAIGenerateReplyWith(r *fastglue.Request, req generateReplyReq) error {
	app := r.Context.(*App)
	auser := r.RequestCtx.UserValue("user").(amodels.User)
	user, err := app.user.GetAgentCachedOrLoad(auser.ID)
	if err != nil {
	...
	if strings.TrimSpace(resp) == "" {
		return sendErrorEnvelope(r, envelope.NewError(envelope.InputError, app.i18n.T("ai.emptyResponse"), nil))
	}
	return r.SendEnvelope(stringutil.Markdown2HTML(resp))
}

func handleGetAIProvider(r *fastglue.Request) error {
	r.RequestCtx.SetUserValue("type", "completion")
	return handleGetAIConfig(r)
}
...
func handleUpdateAIProvider(r *fastglue.Request) error {
	...
	cfg := aimodels.ProviderConfig{APIKey: req.APIKey, Model: req.Model}
	if err := app.ai.UpdateProviderConfig("completion", cfg); err != nil {
		return sendErrorEnvelope(r, err)
	}
	return r.SendEnvelope(true)
}
