...
func initHandlers(g *fastglue.Fastglue, hub *ws.Hub) {
	...
	g.POST("/api/v1/auth/2fa/verify", rateLimit(handleVerifyTwoFactorLogin, "auth"))
	g.GET("/api/v1/users/me/2fa", auth(handleGetTwoFactor))
	g.POST("/api/v1/users/me/2fa/setup", rateLimit(auth(handleSetupTwoFactor), "auth"))
	g.POST("/api/v1/users/me/2fa/enable", rateLimit(auth(handleEnableTwoFactor), "auth"))
	g.POST("/api/v1/users/me/2fa/disable", rateLimit(auth(handleDisableTwoFactor), "auth"))
	g.POST("/api/v1/users/me/2fa/recovery-codes", rateLimit(auth(handleRegenerateTwoFactorCodes), "auth"))
