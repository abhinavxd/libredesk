...
describe('Help Center Form Schema', () => {
      ...
      schema.parse({
        ...
        theme: {
          ...
          layout: {
            collections: 'grid',
            columns: 3,
            show_popular_articles: true,
            popular_articles_label: 'Popular'
          },
          cards: {
  ...
  test('required text fields reject whitespace-only values', () => {
    expect(() => schema.parse({ ...validForm, name: '   ' })).toThrow()
  ...
  test('custom_domain with a path rejected', () => {
    expect(() =>
      schema.parse({ ...validForm, custom_domain: 'https://help.example.com/docs' })
    ).toThrow()
  })
  ...
  test('default_locale must be in allowed_locales', () => {
    expect(() =>
      schema.parse({ ...validForm, default_locale: 'fr', allowed_locales: ['en'] })
    ).toThrow()
  })

  test('invalid locale format', () => {
    expect(() =>
      schema.parse({ ...validForm, default_locale: 'english', allowed_locales: ['english'] })
    ).toThrow()
  })

  test('region locale accepted', () => {
    expect(() =>
      schema.parse({ ...validForm, default_locale: 'pt-BR', allowed_locales: ['pt-BR'] })
    ).not.toThrow()
  })
  ...
  test('theme logo_url protocol relative rejected', () => {
    expect(() =>
      schema.parse({ ...validForm, theme: { logo_url: '//cdn.example.com/l.png' } })
    ).toThrow()
  })

  test('theme logo_url root relative accepted', () => {
    expect(() =>
      schema.parse({ ...validForm, theme: { logo_url: '/uploads/l.png' } })
    ).not.toThrow()
  })
  ...
  test('theme header background_color invalid hex', () => {
    expect(() =>
      schema.parse({ ...validForm, theme: { header: { background_color: 'red' } } })
    ).toThrow()
  })
  ...
  test('theme header background_color accepts 3, 4, 6 and 8 digit hex', () => {
    for (const background_color of ['#fff', '#fff0', '#ffffff', '#ffffff00']) {
      expect(() =>
        schema.parse({ ...validForm, theme: { header: { background_color } } })
      ).not.toThrow()
    }
  })
  ...
  test('nav link requires label and url', () => {
    expect(() =>
      schema.parse({ ...validForm, theme: { nav_links: [{ label: '', url: '/' }] } })
    ).toThrow()
    expect(() =>
      schema.parse({ ...validForm, theme: { nav_links: [{ label: 'Home', url: '' }] } })
    ).toThrow()
    expect(() =>
      schema.parse({ ...validForm, theme: { nav_links: [{ label: 'Home', url: 'home' }] } })
    ).toThrow()
  })
  ...
  test('social link requires platform and url', () => {
    expect(() =>
      schema.parse({
        ...validForm,
        theme: { social_links: [{ platform: '', url: 'https://x.com/a' }] }
      })
    ).toThrow()
    expect(() =>
      schema.parse({ ...validForm, theme: { social_links: [{ platform: 'x', url: 'x.com/a' }] } })
    ).toThrow()
  })

  test('card icon_position invalid value', () => {
    expect(() =>
      schema.parse({ ...validForm, theme: { cards: { icon_position: 'bottom' } } })
    ).toThrow()
  })
